...
from models.icpn_protoAug_semi_2view_blance_add_dualloss_v8_ood_base import IncrementalCPN
...
def main():
    ...
    dual_dataset_ood = get_dual_dataset(dataset=args.dataset, data_path=args.data_path_ood)
    ...
    for task_idx in range(0, args.num_tasks):
        ...
        wandb_logger = WandbLogger(
            name=f"base-{args.perfix}{args.dataset}-{args.pretrained_method}-lambda{args.pl_lambda}-{args.num_tasks}tasks-steps{task_idx}",
        ...
        dual_dataset_task_ood = split_dataset(
            dual_dataset_ood,
            tasks=tasks,
            task_idx=[task_idx],
        )
        ...
        dual_dataset_task_ood = keep_n_samples_per_class(dual_dataset_task_ood,n=args.num_ood)
